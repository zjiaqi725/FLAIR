...
def load_model_only_inference(path, filename, device):
    ...
    if config_sample['max_num_classes'] == 0:
        n_out = 1
    ...
    # print('model_state', model.criterion)
    model.load_state_dict(model_state)   #, strict=False   missing criterion.weight
    model.to(device)
    model.eval()
...
def get_model(config, device, should_train=True, verbose=False, state_dict=None, epoch_callback=None):
    ...
    # print('prior_type', config['prior_type'])
    if config['prior_type'] == 'prior_bag':
        # Prior bag combines priors
        get_batch_gp = make_get_batch(priors.fast_gp)     ### priors.fast_gp.get_batch()
        ...
        prior_hyperparameters = {**get_mlp_prior_hyperparameters(config), **get_gp_prior_hyperparameters(config)
            , **prior_bag_hyperparameters}    ## get_meta_gp_prior_hyperparameters(config)
    ...
    model = train(model_proto.DataLoader
                  ...
                  , epochs=epochs
                  , warmup_epochs=config['warmup_epochs']   #20
                  , bptt=config['bptt']
                  , gpu_device=device
