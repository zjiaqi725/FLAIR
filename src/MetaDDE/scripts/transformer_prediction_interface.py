...
class Normalizer:
    ...
    def inverse_normalize(self, y_norm):
        return y_norm * (self.max_val - self.min_val) + self.min_val
...
def load_model_workflow(i, e, add_name, base_path, device='cpu', eval_addition='', only_inference=True):
    ...
    def get_file(e):
        ...
        """
        for filename in os.listdir('models_diff/'):
            if filename.endswith('.ckpt'):  
                model_file = f'models_diff/{filename}.ckpt'
    ...
    if only_inference:
        ...
        # print('base_path, model_file, device', base_path,'\n', model_file, device)
...
class DynamicDecisionEngine(BaseEstimator, ClassifierMixin):

    models_in_memory = {}

    def __init__(self, device='cpu', base_path=pathlib.Path(__file__).parent.parent.resolve(), model_string='',
                 N_ensemble_configurations=100, no_preprocess_mode=False, multiclass_decoder='permutation',
                 feature_shift_decoder=True, only_inference=True, seed=0, no_grad=True, batch_size_inference=32, ee=None):
        """
        ...
        if model_string in self.models_in_memory:
            ...
            model, c, results_file = load_model_workflow(i, ee, add_name=model_string, base_path=base_path, device=device,
        ...
        # print('models_in_memory', self.models_in_memory[model_key][0]) ##print model 
        self.device = device
        self.model = model
        self.c = c
        self.style = None
    ...
    def fit(self, X, y, overwrite_warning=False, regression=False):
        """
    ...
    def predict_proba(self, X, normalize_with_test=False, return_logits=False):
        ...
        if self.regression:
            prediction = self.normalizer.inverse_normalize(prediction)
    ...
    def predict(self, X, return_winning_probability=False, normalize_with_test=False):
        (X_pmt, y_pmt, X_test) = X 
        self.fit(X_pmt, y_pmt,  regression=True)  #.detach().cpu().numpy()
        if self.regression:
            y = self.predict_proba(X_test, normalize_with_test=normalize_with_test)   #.detach().cpu().numpy()
            return y
        else:
            p = self.predict_proba(X_test, normalize_with_test=normalize_with_test)
...
#                return 'robust_all'
    ...
    feature_shift_configurations = torch.randperm(eval_xs.shape[2]) if (feature_shift_decoder and not regression) else [0]
    class_shift_configurations = torch.randperm(len(torch.unique(eval_ys))) if (multiclass_decoder == 'permutation' and not regression) else [0]
    ...
    # print('ensemble_configurations', ensemble_configurations)
    ...
    if average_logits and not return_logits and not regression:
        output = torch.nn.functional.softmax(output, dim=-1)
