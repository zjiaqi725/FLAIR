...
class TransformerModel(nn.Module):
    def __init__(self, encoder, n_out, ninp, nhead, nhid, nlayers, dropout=0.0, style_encoder=None, y_encoder=None,
        ...
        # print('nlayers', nlayers) nlayers=12 12个transformer encoder
        self.transformer_encoder = TransformerEncoder(encoder_layer_creator(), nlayers)\
            if all_layers_same_init else TransformerEncoderDiffInit(encoder_layer_creator, nlayers)
        self.ninp = ninp
        self.encoder = encoder #Linear(in_features=100, out_features=512, bias=True)
        self.y_encoder = y_encoder
        self.pos_encoder = pos_encoder
        # if n_out == 1:
        #     self.decoder = decoder(ninp, nhid, n_out) if decoder is not None else nn.Linear(ninp, n_out)
        # else:
    ...
    def generate_square_subsequent_mask(sz):
        ...
        return bool_mask_to_att_mask(mask)   #生成大小为sz的下三角mask
    ...
    # sz = train_size + query_size(test_size)
    ...
    # generate_global_att_query_matrix(3,3,2) test/query部分的mask
    ...
    def forward(self, src, src_mask=None, single_eval_pos=None):
        ...
        y_src = self.y_encoder(y_src.unsqueeze(-1) if len(y_src.shape) < len(x_src.shape) else y_src)   #linear(1,emsize)
        ...
        # x_src/y_src [n_sample, 1, 512(emsize)]
        # print('style_src', style_src, global_src)
        train_x = x_src[:single_eval_pos] + y_src[:single_eval_pos]
        src = torch.cat([global_src, style_src, train_x, x_src[single_eval_pos:]], 0) 
        # print('input', single_eval_pos, src.shape, train_x.shape, x_src[:single_eval_pos].shape, y_src[:single_eval_pos].shape)
        # input 48 torch.Size([1152, 1, 512]) torch.Size([48, 1, 512]) torch.Size([48, 1, 512]) torch.Size([48, 1, 512])
        # <train_x, x_src[single_eval_pos:]> = <x_train+y_train, x_test>    single_eval_pos train_set_size

        if self.input_ln is not None:
            src = self.input_ln(src)
        
        if self.pos_encoder is not None:
            # print('self.pos_encoder', self.pos_encoder) # NoPositionalEncoding() input=output
            src = self.pos_encoder(src) 

        # print('src', self.full_attention, self.efficient_eval_masking ) #, src.shape = torch.Size([1152, 1, 512]) 
        # since self.efficient_eval_masking, src_mask is a int
        output = self.transformer_encoder(src, src_mask)  #TransformerEncoderDiffInit(encoder_layer_creator, nlayers)
#         (layers): ModuleList(
