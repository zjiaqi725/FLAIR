...
def get_batch(batch_size, seq_len, num_features, device=default_device
    ...
    prior_bag_priors_get_batch = hyperparameters['prior_bag_get_batch']  ## len=2 (get_batch_gp, get_batch_mlp)
    prior_bag_priors_p = [1.0] + [hyperparameters[f'prior_bag_exp_weights_{i}'] for i in range(1, len(prior_bag_priors_get_batch))]
    # prior_bag_priors_p = [1.0] + [1.0 for i in range(1, len(prior_bag_priors_get_batch))]
    # print('prior_bag_priors_p', hyperparameters['prior_bag_exp_weights_1'], [hyperparameters[f'prior_bag_exp_weights_{i}'] for i in range(1, len(prior_bag_priors_get_batch))])
    
    #  samples from a prior bag using a multinomial distribution based on prior weights.
