...
def train(priordataloader_class, criterion, encoder_generator, emsize=200, nhid=200, nlayers=6, nhead=2, dropout=0.0,
          epochs=10, steps_per_epoch=100, batch_size=200, bptt=10, lr=None, lr_enc=None, weight_decay=0.0, warmup_epochs=10, input_normalization=False,
    ...
    # synethtic data
    dl = priordataloader_class(num_steps=steps_per_epoch, batch_size=batch_size, 
                               eval_pos_seq_len_sampler=eval_pos_seq_len_sampler, 
                               seq_len_maximum=bptt+(bptt_extra_samples if bptt_extra_samples else 0), 
                               device=device, **extra_prior_kwargs_dict)
    # print('dl.num_features',dl.num_features) #predefined num_features=100
    ...
    if isinstance(criterion, nn.GaussianNLLLoss):
        ...
        n_out = criterion.weight.shape[0]   ##类别数
    elif isinstance(criterion, (nn.MSELoss,nn.SmoothL1Loss)):
        n_out = 1    ##regression
    else:
        n_out = 1
    
    # print('y_encoder', encoder)
    ...
    if load_weights_from_this_state_dict is not None:
        ...
        model_state_dict = {}
        for key in load_weights_from_this_state_dict:
            # print('key', key)
            if key.startswith('decoder.2'):  #decoder last layer
                print('key', key)
            elif key.startswith('criterion'):  #criterion.weight
                print('key', key)
            else:
                model_state_dict[key] = load_weights_from_this_state_dict[key]
    ...
    for name, param in model.named_parameters():
        if 'decoder' not in name:
            model_para_enc.append(param)
    
    optimizer_encoder = torch.optim.AdamW(model_para_enc, lr=lr_enc, weight_decay=weight_decay)
    optimizer_decoder = torch.optim.AdamW([param for param in model.decoder.parameters()], lr=lr_dec, weight_decay=weight_decay)
    # print('warmup_epochs', warmup_epochs)
    scheduler_enc = scheduler(optimizer_encoder, warmup_epochs, epochs if epochs is not None else 100, num_cycles=0.5)
    scheduler_dec = scheduler(optimizer_decoder, warmup_epochs, epochs if epochs is not None else 100, num_cycles=0.5)
    ...
    def train_epoch():
        ...
        for batch, (data, targets, single_eval_pos) in enumerate(dl):
            # print('batch', batch)
            # print('data', batch, data[0], data[1].shape, data[2][0].shape, targets[0].shape, single_eval_pos)  #isinstance(data, tuple) data=(style=None,x,y)
            # are_equal = torch.equal(data[2], targets)  data[2]=y=targets
            # print('!!', are_equal)  #data[2]=target
            # print('datadata', data[1].shape, data[-1].shape, targets.shape)
            # print('datadata', data[-1], targets)
            
            if using_dist and not (batch % aggregate_k_gradients == aggregate_k_gradients - 1):
                cm = model.no_sync()
            else:
                cm = nullcontext()  #Context Manager
            with cm:
                ...
                with autocast(enabled=scaler is not None):  #自动混合精度，适用于大模型或大数据集
                    ...
                    elif isinstance(criterion, (nn.MSELoss,nn.SmoothL1Loss, nn.BCEWithLogitsLoss)):
                        # print('output:',  output, output.shape)
                        # print('target:', targets, targets.shape)
                        losses = criterion(output.flatten(), targets.to(device).flatten())
                        # losses = torch.sqrt(losses)
                        # if torch.any(torch.isnan(losses)):
                        #     print('output:',  output, output.shape)
                        #     print('target:', targets, targets.shape)
                        #     print('losses', losses)
                    elif isinstance(criterion, nn.CrossEntropyLoss):
                    ...
                    loss, nan_share = utils.torch_nanmean(losses.mean(0), return_nanshare=True)  #忽略NaN计算均值，并返回均值和被忽略的NaN值的比例
                    loss = loss / aggregate_k_gradients
                    # if torch.isnan(loss):
                    #     print('loss', loss, torch.isnan(loss))

                if scaler: loss = scaler.scale(loss)
                loss.backward()

                if batch % aggregate_k_gradients == aggregate_k_gradients - 1:
                    if scaler: scaler.unscale_(optimizer_decoder)
                ...
                if not torch.isnan(loss):
                    ...
                    #     print('bptt', torch.ones(bptt))
                    # else:
                    #     print('onehot', nn.functional.one_hot(torch.tensor(single_eval_pos), bptt))
            ...
            before_get_batch = time.time()
            total_positional_losses_recorded = torch.tensor(total_positional_losses_recorded) + 1e-7
    ...
    try:
        for epoch in (range(1, epochs + 1) if epochs is not None else itertools.count(1)):
            ...
            total_loss, total_positional_losses, time_to_get_batch, forward_time, step_time, nan_share, ignore_share =\
                train_epoch()
            # print('float', type(total_positional_losses))
            if isinstance(total_positional_losses, float):
                print(f"NaN loss detected in epoch {epoch}. Skipping this epoch.")
                continue 
            # print('total_positional_losses', total_positional_losses)
            ...
            if verbose:
                ...
                print(
                    ...
                    f"pos losses {','.join([f'{l:5.2f}' for l in total_positional_losses])}, lr {scheduler_dec.get_last_lr()[0]}"
                    f' data time {time_to_get_batch:5.2f} step time {step_time:5.2f}'
                    f' forward time {forward_time:5.2f}' 
                    f' nan share {nan_share:5.2f} ignore share (for classification tasks) {ignore_share:5.4f}'
                    + (f'val score {val_score}' if val_score is not None else ''))
                print('-' * 89)
            # current_lr_enc = scheduler_enc.get_last_lr()[0]
            # current_lr_dec = scheduler_dec.get_last_lr()[0]
            # print(f"Epoch {epoch}/{epochs}, Learning Rate encoder: {current_lr_enc}")
            # print(f"Epoch {epoch}/{epochs}, Learning Rate decoder: {current_lr_dec}")
